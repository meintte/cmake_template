...
if(SLU_UTIL_HEADER)
  ...
  string(
    REGEX
    REPLACE ".*#define SUPERLU_MAJOR_VERSION[ \t]+([0-9]+).*"
            "\\1"
            SUPERLU_MAJOR_VERSION
            "${superluheader}")
  string(
    REGEX
    REPLACE ".*#define SUPERLU_MINOR_VERSION[ \t]+([0-9]+).*"
            "\\1"
            SUPERLU_MINOR_VERSION
            "${superluheader}")
  string(
    REGEX
    REPLACE ".*#define SUPERLU_PATCH_VERSION[ \t]+([0-9]+).*"
            "\\1"
...
if(SuperLU_FOUND)
  if(NOT TARGET SuperLU::SuperLU)
    ...
    set_target_properties(SuperLU::SuperLU PROPERTIES IMPORTED_LOCATION ${SUPERLU_LIBRARY} INTERFACE_INCLUDE_DIRECTORIES
                                                                                           "${SUPERLU_INCLUDE_DIR}")
    # Link BLAS library
    if(TARGET BLAS::BLAS)
